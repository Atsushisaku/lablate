...
| ストレージ（暫定） | localStorage + IndexedDB（画像） |
...
- **複数系列プロット（Y列複数選択、8色カラーパレット、凡例）**
- **多項式回帰（1〜3次）と R² 表示（系列ごとON/OFF、同系色破線）**
- **グラフ詳細設定（データ範囲）の折りたたみ**
- **ページリンクブロック（Notion風、`/ページリンク` で挿入）**
- **スプレッドシートタブの操作・UIをmd内と統一**
- 詳細仕様：[PHASE2.md](./PHASE2.md)

---

### フェーズ3：画像挿入・ストレージ抽象化
**目標**：画像を扱えるようにし、SaaS化に向けたストレージ設計を整える

- 画像ブロック（D&D / ペースト / ファイル選択で挿入）
- クライアント側での画像リサイズ・圧縮
- IndexedDB による画像保存（ローカル動作）
- ストレージインターフェースの定義（将来の OneDrive / サーバー差し替えポイント）
- 詳細仕様：[PHASE3.md](./PHASE3.md)

---

### フェーズ4：OneDrive連携・認証・SaaS基盤
**目標**：組織展開できる状態にする

- Microsoftアカウントでログイン（Azure AD）
- 全ストレージを StorageProvider 経由に統一
