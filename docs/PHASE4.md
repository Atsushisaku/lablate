...
OneDrive / Google Drive 等の同期フォルダを保存先に指定すれば、クラウド同期も自動的に実現される。
**Lablate 側でデータサーバーを用意しない**設計を維持する。
...
- フォルダ未接続 + 初回起動時に、軽いガイドバナーを表示
  ...
  - 「OneDrive フォルダを選択すると、複数 PC で同期できます」
  - 「後で設定」で閉じられる
...
- OneDrive / Google Drive API 直接連携（OAuth 認証）
